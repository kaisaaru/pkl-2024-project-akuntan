<?php

namespace App\Models;

use App\Models\faktur_jual;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Concerns\HasUuids;

class fj_line extends Model
{
    use HasFactory;

    protected $hidden = [
        'remember_token',
    ];
    protected $guarded = [
        'id',
        '_token',
    ];
    protected $dates = ['created_at'];
    protected $fillable = ['user_id','hariini'];

    protected $table = 'fj_lines';
  
    protected static function boot() {
        parent::boot();

        // Menambahkan event creating untuk mengatur id_fb otomatis
        static::creating(function ($faktur_jual) {
            $lastId = self::max('id'); // Mendapatkan id tertinggi dari tabel penerima$faktur_beli
            $nextId = $lastId + 1; // Menghitung id berikutnya

            if($nextId < 10) {
                // Mengatur id_fb dengan format "B00(id)"
                $faktur_jual->id_fj = 'FJ-000'.$nextId.'';
            } else if($nextId < 100) {
                // Mengatur id_fb dengan format "B00(id)"
                $faktur_jual->id_fj = 'FJ-00'.$nextId.'';
            } else if($nextId < 1000) {
                // Mengatur id_fb dengan format "B00(id)"
                $faktur_jual->id_fj = 'FJ-0'.$nextId.'';
            }
        });

        static::creating(function ($detail_fj) {
            $lastId = self::max('id'); // Mendapatkan id tertinggi dari tabel detail_pb
            $nextId = $lastId + 1; // Menghitung id berikutnya

            if($nextId < 10) {
                // Mengatur id_detail_pb dengan format "B00(id)"
                $detail_fj->id_detailfj = 'D.FJ-000'.$nextId.'';
            } else if($nextId < 100) {
                // Mengatur id_detail_pb dengan format "B00(id)"
                $detail_fj->id_detailfj = 'D.FJ-00'.$nextId.'';
            } else if($nextId < 1000) {
                // Mengatur id_detail_pb dengan format "B00(id)"
                $detail_fj->id_detailfj = 'D.FJ-0'.$nextId.'';
            }
        });
    }

    public function fakturjual()
    {
        return $this->hasMany(faktur_jual::class);
    }
}
